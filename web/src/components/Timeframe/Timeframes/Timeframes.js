...
import { QUERY } from 'src/components/Timeframe/TimeframesCell'
      ...
      <table className="rw-table">
        ...
        <tbody>
          {timeframes.map((timeframe) => (
            <tr key={timeframe.id}>
              <td>{truncate(timeframe.id)}</td>
              <td>{truncate(timeframe.name)}</td>
              <td>{truncate(timeframe.days)}</td>
              <td>{truncate(timeframe.monthSplits)}</td>
              <td>{timeTag(timeframe.date)}</td>
              <td>{timeTag(timeframe.deleted)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.timeframe({ id: timeframe.id })}
                    title={'Show timeframe ' + timeframe.id + ' detail'}
                  ...
                  <Link
                    ...
                    title={'Edit timeframe ' + timeframe.id}
                  ...
                  <button
                    type="button"
                    title={'Delete timeframe ' + timeframe.id}
